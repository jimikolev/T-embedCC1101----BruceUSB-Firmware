...
#include <globals.h>
#include "core/SerialInputHandler.h"
...
extern SerialInputHandler inputHandler;  // define the global instance 
...
void boot_screen() {
    ...
    tft.drawCentreString("BruceUSB", tftWidth / 2, 10, 1);
    tft.setTextSize(FP);
...
void loop() {
    ...
    if (Serial.available()) {
        ...
        if (input == "NEXT") NextPress = true;
        else if (input == "PREV") PrevPress = true;
        else if (input == "UP") UpPress = true;
        else if (input == "DOWN") DownPress = true;
        else if (input == "SEL") SelPress = true;
        else if (input == "ESC") EscPress = true;
        else if (input == "LONGPRESS") LongPress = true;
