...
private:
    ...
    bool _logging = false;
    void clearLog();
    uint16_t* framebuffer = nullptr; // <- Framebuffer pointer this is your serial frame buffer type shit

public:
    tft_logger(int16_t w = TFT_WIDTH, int16_t h = TFT_HEIGHT);
    virtual ~tft_logger();
    void sendBitmapBlock(int32_t x, int32_t y, int32_t w, int32_t h);
